...
KILL_COOLDOWN    = 1800   # 30 minutes
ROB_COOLDOWN     = 600    # 10 minutes
_ECO_KEY         = "economy"
...
async def bal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    from handlers.shop import get_inv
    inv    = get_inv(target.id)
...
async def kill_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    eco  = get_eco(user.id)
    now  = time.time()
    diff = now - eco.get("last_kill", 0)
    if diff < KILL_COOLDOWN:
        return await msg.reply_text(
            f"⏳ /kill cooldown!\nYou can kill again in <b>{_fmt_cd(int(KILL_COOLDOWN - diff))}</b>.",
            parse_mode="HTML",
        )
...
async def rob_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    eco  = get_eco(user.id)
    now  = time.time()
    diff = now - eco.get("last_rob", 0)
    if diff < ROB_COOLDOWN:
        return await msg.reply_text(
            f"⏳ /rob cooldown!\nYou can rob again in <b>{_fmt_cd(int(ROB_COOLDOWN - diff))}</b>.",
            parse_mode="HTML",
        )
