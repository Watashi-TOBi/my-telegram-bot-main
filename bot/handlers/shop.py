...
async def shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Weapons list ───────────────────────────────────────────────────────────
        ...
        for wid, w in WEAPONS.items():
            lo, hi = w["dmg"]
            if wid in owned:
                label = f"{'✓ ' if wid == eq_w else ''}✅ {w['name']} — Equip"
                rows.append([InlineKeyboardButton(label, callback_data=f"sh|ew|{wid}")])
            else:
    ...
    # ── Inventory ──────────────────────────────────────────────────────────────
        ...
        await q.edit_message_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(_back_kb()))
    ...
    # ── Buy weapon ─────────────────────────────────────────────────────────────
        ...
        for _wid, _w in WEAPONS.items():
            lo, hi = _w["dmg"]
            if _wid in owned:
                label = f"{'✓ ' if _wid == eq_w else ''}✅ {_w['name']} — Equip"
                rows.append([InlineKeyboardButton(label, callback_data=f"sh|ew|{_wid}")])
            else:
                rows.append([InlineKeyboardButton(
    ...
    # ── Equip weapon ───────────────────────────────────────────────────────────
        ...
        for _wid, _w in WEAPONS.items():
            lo, hi = _w["dmg"]
            if _wid in owned:
                label = f"{'✓ ' if _wid == wid else ''}✅ {_w['name']} — Equip"
                rows.append([InlineKeyboardButton(label, callback_data=f"sh|ew|{_wid}")])
            else:
                rows.append([InlineKeyboardButton(
                    f"{_w['name']} [{_w['type']}] {_w['price']:,}🪙 | {lo}-{hi} dmg",
