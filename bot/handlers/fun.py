...
_BANNED_WITH_RASM = {
    "boys", "lgbtq", "gay", "chakke", "chakka", "dick", "cock", "sucks_cock",
}


def _is_rasm_banned_pair(name1: str, name2: str) -> bool:
    n1, n2 = name1.lower().strip(), name2.lower().strip()
    if n1 == "rasm" and n2 in _BANNED_WITH_RASM:
        return True
    if n2 == "rasm" and n1 in _BANNED_WITH_RASM:
        return True
    return False
...
async def love_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _is_rasm_banned_pair(name1, name2):
        await update.message.reply_text(
            ...
            f"━━━━━━━━━━━━━━━━\n"
            f"<code>[░░░░░░░░░░]</code>  <b>0%</b>\n\n"
            f"🚫  <i>Not a chance. Never. Absolutely zero. Disgusting combo.</i>",
            parse_mode="HTML",
        )
        return
