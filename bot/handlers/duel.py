    ...
    if action == "acc":
        ...
        if bet > 0:
            if get_coins(c_id) < bet:
                await q.edit_message_text(f"❌ {c_name} no longer has enough coins for the bet!")
                return
            if get_coins(t_id) < bet:
                await q.edit_message_text(f"❌ {t_name} doesn't have enough coins for the bet!")
                return
...
async def _run_duel(
    ...
    if a_hp > b_hp:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="🤝 <b>The duel ended in a DRAW!</b>\n\nBoth fighters are still standing!",
            parse_mode="HTML",
        )
        return
