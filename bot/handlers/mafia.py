...
async def _start_night(context, chat_id: int) -> None:
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=(
            f"🌙 <b>Night {round_n} falls...</b>\n\n"
            f"The town goes to sleep. The Mafia is plotting... 🔴\n"
            f"<i>Check your DMs to take your night action.</i>"
        ),
        parse_mode="HTML",
    )
    ...
    if mafia:
        ...
        if failed:
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"⚠️ Couldn't DM {', '.join(failed)}. They need to /start the bot in DM first.",
            )
...
async def resend_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ok:
        ...
        await update.message.reply_text(
            "❌ Couldn't DM you. Please start the bot in DM first: open a chat with me and press Start."
        )
...
async def gamerules_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "/resend · /players · /skip · /endgame · /gamerules",
        parse_mode="HTML",
    )
